...
describe('variable modifiers', () => {
  ...
  it('checks whether the number of colors is ok and corrects it if not', () => {
    ...
    let arcColors = ['red']
    expect(gauge.arcColorsModifier(arcDelimiters, arcColors))
                .toEqual(['red', schemePaired[0], schemePaired[1]])

    arcDelimiters = [2]
    arcColors = ['red', 'blue', 'green']
    expect(gauge.arcColorsModifier(arcDelimiters, arcColors)).toEqual(['red', 'blue'])
...
describe('arc outlining', () => {
  ...
  it ('checks arc svg outline', () => {
    ...
    let arcDelimiters = []
    let arcColors = ['red']
    gauge.arcOutline(svg, chartHeight, offset, arcColors, outerRadius, arcDelimiters)
...
describe('label outlining', () => {
  ...
  it ('checks correct text of gauge with central label', () => {
    ...
    expect(svgHtml[0]).toBe('<text x="0" y="106.8" font-size="14px"' +
                       ' font-family="Roboto,Helvetica Neue,sans-serif">')
    expect(svgHtml[1]).toBe('<text x="0" y="106.8" font-size="14px"' +
                       ' font-family="Roboto,Helvetica Neue,sans-serif">')
    expect(svgHtml[2]).toBe('<text x="94.12" y="90" font-size="21px"' +
  ...
  it ('checks correct text of gauge with range labels', () => {
    ...
    let svgHtml = svg.html().split('</text>')
    svgHtml.pop()  // removed last element (empty string)
    expect(svgHtml[0]).toBe('<text x="25.8" y="106.8" font-size="14px"' +
                       ' font-family="Roboto,Helvetica Neue,sans-serif">' + rangeLabel[0])
    expect(svgHtml[1]).toBe('<text x="165.8" y="106.8" font-size="14px"' +
                       ' font-family="Roboto,Helvetica Neue,sans-serif">' + rangeLabel[1])
    expect(svgHtml[2]).toBe('<text x="100" y="90" font-size="21px"' +
                       ' font-family="Roboto,Helvetica Neue,sans-serif">')
  })
...
describe('console warnings and errors', () => {
  it ('spies an error about delimiters range', () => {
    ...
    let message = 'Gauge-chart Error: gauge delimiters have to be LARGER than 0 and LESS than 100'
